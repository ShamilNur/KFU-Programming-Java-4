document.addEventListener('DOMContentLoaded', () => {
    ...
    let popularRooms = document.querySelector("#popular-rooms-btn");
    console.log(popularRooms)
    popularRooms.addEventListener('click', () => {
        console.log("addEventListener")
        getMostPopularRooms()
    });

    function getMostPopularRooms() {
        // Выполняем Ajax-запрос.
        $.ajax({
            type: "POST",
            url: "/availability/popular",
            data: JSON.stringify({}),
            success: response => {
                console.log('response: ', response)
                renderPopularRoomsSection(response, $('#popular-rooms'))
            },
            dataType: "json",
    ...
    function renderPopularRoomsSection(rooms, popularRooms) {
        let innerHtml = '';
    ...
    function sendAvailabilityForm(dateRange, roomsNumber, adultsNumber, childNumber) {
        ...
        // Собираем данные для отправки в карту.
        let data = {
            'dateFrom': dateFrom,
            'dateTo': dateTo,
    ...
    function renderAvailableRoomsSection(rooms, availableRooms) {
        ...
        for (let item of rooms) {
            ...
            innerHtml += `                <p>${item['roomsNumber']} places</p>`;
            innerHtml += '            </div>';
            innerHtml += '            <form method="post" action="/booking">';
            innerHtml += '                <input type="hidden" name="id" value="' + item['id'] + '">';
            innerHtml += '                <button type="submit" class="primary-btn">Book</button>';
            innerHtml += '            </form>';
            innerHtml += '        </div>';
            innerHtml += '    </div>';
            innerHtml += '</div>';
